...
const ContextProvider = (props) => {
  const [adminContext, setAdminContext] = useState({ isAuthenticated: false });
  ...
  const Login = async (email, password) => {
    ...
    if (loginResult.success) {
      ...
      setPublicContext({ ...publicContext, chapters: loginResult.chapters });

      AddToken(loginResult.token);
      SavePublicDataToLocalStorage(publicContext);
