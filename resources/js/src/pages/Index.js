...
const Index = () => {
  ...
  return (
    <React.Fragment>
      ...
      <Context.Consumer>
        {(context) => {
          const books = context.public.books;
          const rand_book = getRandom(books, 8);
          const pop_books = rand_book.splice(0, 4);
          const new_books = rand_book;

          const best_authors = getRandom(context.public.authors, 4);
          const best_genres = getRandom(context.public.genres, 6);

          const last_articles = getRandom(context.public.posts, 3);
              ...
              <Grid container spacing={1}>
                {best_authors.map((author) => (
                  <Grid key={author.id} item xs={6} sm={3}>
                    <AuthorThumbnail title={author.name} img={author.avatar} />
                  </Grid>
                ))}
              </Grid>
